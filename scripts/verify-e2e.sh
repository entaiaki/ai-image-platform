...
for i in {1..60}; do
  ...
  STATUS=$(echo "$RES" | py -c "import sys,json;print(json.load(sys.stdin)['data']['status'])")
  OUT_URL=$(echo "$RES" | py -c "import sys,json;d=json.load(sys.stdin)['data'];print(d.get('outputImageUrl') or '')")
  OUT_PATH=$(echo "$RES" | py -c "import sys,json;d=json.load(sys.stdin)['data'];print(d.get('outputLocalPath') or '')")
...
FOUND=$(echo "$HIS" | py -c "import sys,json;arr=json.load(sys.stdin)['data'];tid=int('$TASK_ID');print('1' if any(int(x['id'])==tid for x in arr) else '0')")
